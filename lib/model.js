...
class Model extends EventEmitter {
    ...
    constructor({
        ...
        this.metaFile = path.resolve(this.folder, this.filename)
        this.meta = null
        this.isNew = false
        
        this.__init(this.folder)
    }
    ...
    get(key, {
        ...
        try {
            ...
            if (event) {
                this.emit('error', __funcname, error, { key })
            } else {
    ...
    set(key, value = {}, index = undefined, {
        ...
        try {
            ...
            if (event) {
                this.emit('error', __funcname, error, { key, value, index })
            } else {
                throw new Error(error && error.message || 'Fatal Error')
            }
        }
    ...
    find(comparator = (obj) => { return false }) {
        ...
        if (this.meta && this.meta.data) {
            for (const [key, index = {}] of Object.entries(this.meta.data)) {
                const file = this.__get_file_path(key)
                if (comparator(index) && fs2.pathExistsSync(file)) {
                    const data = fs2.readJsonSync(file) || null
                    if (data) {
                        result = { key, data, options: index }
                        break
                    }
                }
            }
    ...
    findAll(comparator = (obj) => { return true }) {
        ...
        for (const [key, index = {}] of Object.entries(this.meta.data || {})) {
            ...
            if (comparator(index) && fs2.pathExistsSync(file)) {
                const data = fs2.readJsonSync(file) || null
                if (data) result.push({ key, data, options: index })
            }
        }
        return result
    }
    ...
    mset(data, {
        ...
        try {
            ...
            if (event) {
                this.emit('error', __funcname, error, data)
            } else {
                throw new Error(error && error.message || 'Fatal Error')
            }
        }
