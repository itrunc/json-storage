...
class Schema extends EventEmitter {
    ...
    __get_folder_path(name) {
        const folder = path.resolve(this.folder, this.__key__(name))
    ...
    __del(type, name, {
        ...
        try {
            const key = this.__key__(name)
            const meta = this.__meta__(type)
            const folder = this.__get_folder_path(key)
    ...
    model(name, index) {
        ...
        try {
            ...
            const result = new Model({
                folder: path.resolve(this.folder, key)
            })
